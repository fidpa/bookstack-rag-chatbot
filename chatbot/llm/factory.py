...
def create_llm_provider(provider_type: Optional[str] = None) -> LLMProvider:
    ...
    if provider_type == "ollama":
        if not ENABLE_OLLAMA:
            raise ValueError(
                "Ollama ist deaktiviert. Setzen Sie ENABLE_OLLAMA_FALLBACK=true um Ollama zu aktivieren."
            )
        ...
        if not api_key or not endpoint:
            raise ValueError(
                "Azure provider requested but credentials not found (AZURE_OPENAI_API_KEY and AZURE_OPENAI_ENDPOINT required)"
            )
        ...
        if ENABLE_OLLAMA:
            try:
                ...
                if ollama.is_available():
                    logger.info(
                        "Auto-selected Ollama provider (ENABLE_OLLAMA_FALLBACK=true)"
                    )
            ...
            logger.info(
                "Ollama fallback disabled (set ENABLE_OLLAMA_FALLBACK=true to enable)"
            )

        # No providers available - raise error
        if ENABLE_OLLAMA:
            raise ValueError(
                "Kein LLM-Provider verfügbar. Bitte Azure OpenAI konfigurieren oder Ollama starten."
            )
        else:
            raise ValueError(
                "Azure OpenAI nicht verfügbar. Ollama ist deaktiviert (ENABLE_OLLAMA_FALLBACK=false)."
            )
