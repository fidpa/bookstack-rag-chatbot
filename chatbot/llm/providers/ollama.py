...
class OllamaProvider(LLMProvider):
    ...
    def __init__(
        self, model_name: str = "llama3.2:1b", base_url: str = "http://localhost:11434"
    ):
        """
        ...
        """
        super().__init__("ollama", {"model_name": model_name, "base_url": base_url})
    ...
    def is_available(self) -> bool:
        ...
        try:
            ...
            available = any(
                ...
                or self.model_name.split(":")[0] == name.split(":")[0]
    ...
    def chat(self, messages: List[Dict[str, str]], **kwargs) -> str:
        ...
        try:
            ...
            if "mistral-nemo" in self.model_name.lower():
                dynamic_timeout = max(
                    120, min(300, total_chars // 100)
                )  # Min 120s for Mistral-Nemo
            else:
                dynamic_timeout = max(
                    60, min(300, total_chars // 200)
                )  # Min 60s, max 300s

            logger.debug(f"Using timeout of {dynamic_timeout}s for {total_chars} chars")
            logger.info(
                f"Sending request to Ollama with num_gpu: {data['options'].get('num_gpu', 'not set')}"
            )

            response = requests.post(
                f"{self.base_url}/api/chat", json=data, timeout=dynamic_timeout
            )
            ...
            raise Exception(
                "Die Anfrage an Ollama hat zu lange gedauert. Bitte versuchen Sie es erneut."
            )
        except requests.exceptions.RequestException as e:
