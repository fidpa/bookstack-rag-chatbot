...
class SearchService:
    ...
    @classmethod
    def search_documents(cls, query: str, page: int = 1, per_page: int = 20, 
                        active_only: bool = True, use_hybrid: bool = True):
        """
        ...
        if use_hybrid:
            try:
                ...
                # Logge Such-Info
                logger.info(f"Hybrid-Search für '{query}': {search_info.get('unique_documents', 0)} eindeutige Dokumente gefunden")
                
                return documents, total_count, search_info
            except Exception as e:
                logger.error(f"Fehler bei Hybrid-Search, falle zurück auf Legacy: {str(e)}")
        ...
        try:
            with get_db_connection() as conn:
                ...
                logger.info(f"Suche nach '{query}' (escaped: '{escaped_query}') ergab {total_count} Treffer (Seite {page})")
    ...
    @classmethod
    def get_all_documents(cls, page: int = 1, per_page: int = 20, 
                         only_active: Optional[bool] = None) -> Tuple[List[KnowledgeDocument], int]:
        """
    ...
    def toggle_document_status(cls, doc_id: int) -> Tuple[bool, str]:
        ...
        try:
            with get_db_connection() as conn:
                ...
                cursor.execute('SELECT is_active FROM kb_documents WHERE id = ?', (doc_id,))
                row = cursor.fetchone()
