            ...
            try:
                with get_db_connection() as conn:
                    ...
                    for doc_data in doc_results.values():
                        ...
                        match_boost = min(
                            doc_data["chunk_count"] / 5, 2
                        )  # Boost für viele Matches

                        base_score = cls.STRATEGY_WEIGHTS[SearchStrategy.CHUNK_BASED]
                        doc_data["result"].relevance_score = (
                            base_score * match_boost * (1 / (1 + avg_rank))
                        )
                ...
                logger.error(
                    f"Fehler bei Chunk-Suche für Query '{log_query}': {str(e)}"
                )
            ...
            with get_db_connection() as conn:
                ...
                for term in terms:
                    if len(term) >= 4:  # Nur für längere Begriffe
                        fuzzy_terms.append(
                            f"{QueryProcessor.preprocess_for_fts5(term)}*"
                        )
