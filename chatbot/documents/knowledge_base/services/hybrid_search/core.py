...
class HybridSearchService:
    ...
    def search(cls, query: str, page: int = 1, per_page: int = 20,
              active_only: bool = True) -> Tuple[List[KnowledgeDocument], int, Dict]:
        """
        ...
        logger.info(f"Query-Analyse: Intent={analysis.intent.value}, "
    ...
    @classmethod
    def _execute_multi_strategy_search(cls, analysis: QueryAnalysis, 
                                     active_only: bool) -> Dict[SearchStrategy, List[SearchResult]]:
        ...
        if len(analysis.keywords) >= 2:
            ...
            exact_results = SearchImplementations.search_exact_phrase(phrase, active_only)
            if exact_results:
                results[SearchStrategy.EXACT_PHRASE] = exact_results
        
        # 3. Keyword Search (OR)
        if analysis.keywords:
            or_results = SearchImplementations.search_keywords_or(analysis.keywords, active_only)
            if or_results:
        ...
        chunk_results = ExtendedSearchImplementations.search_chunks(analysis, active_only)
        if chunk_results:
        ...
        if analysis.keywords:
            ...
            if bookstack_content_results:
                results[SearchStrategy.KEYWORD_OR] = results.get(SearchStrategy.KEYWORD_OR, []) + bookstack_content_results
        ...
        if analysis.keywords:
            ...
            if bookstack_chunk_results:
                results[SearchStrategy.CHUNK_BASED] = results.get(SearchStrategy.CHUNK_BASED, []) + bookstack_chunk_results
