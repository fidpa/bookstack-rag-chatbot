...
class ChunkSelectionStrategy:
    ...
    @classmethod
    def build_context(cls, documents: List[KnowledgeDocument], query: str, max_tokens: int = 3000) -> str:
        """
        ...
        try:
            ...
            for doc_idx, (doc_id, doc_data) in enumerate(doc_chunks_map.items(), 1):
                ...
                for chunk_idx, chunk in enumerate(doc_data['chunks'][:3], 1):  # Max 3 Chunks pro Dokument
            ...
            full_context = "\n".join(context_parts)
            logger.info(f"Chunk-basierter Kontext erstellt: {len(doc_chunks_map)} Dokumente, {len(all_chunks)} Chunks, {total_tokens} Tokens")
            
            return full_context
            
        except Exception as e:
            logger.error(f"Fehler beim Erstellen des Chunk-basierten Kontexts: {str(e)}", exc_info=True)
    ...
    def _format_simple_context(cls, documents: List[KnowledgeDocument]) -> str:
        ...
        for idx, doc in enumerate(documents[:3], 1):  # Max 3 Dokumente
            ...
            context_parts.append(f"\n### Dokument {idx}: {title}")
            context_parts.append(snippet[:500] + "..." if len(snippet) > 500 else snippet)
