...
class DocumentSelectionStrategy:
    ...
    def build_context(cls, documents: List[KnowledgeDocument], user_query: str) -> str:
        ...
        for idx, doc in enumerate(documents, 1):
            logger.debug(
                f"Adding document {idx} to context: {doc.title or doc.original_filename}"
            )
        ...
        logger.info(
            f"Kontext mit {len(documents)} Dokumenten erstellt für Query: {user_query} (Länge: {len(full_context)} Zeichen)"
        )
            ...
            try:
                with get_db_connection() as conn:
                    ...
                    if row:
                        ...
                        logger.debug(
                            f"Fetched content for doc {doc.id}: {len(snippet)} chars"
                        )
                    else:
                        logger.warning(
                            f"No content found in search index for doc {doc.id}"
                        )
