...
class ContextService:
    ...
    def build_knowledge_context(
        cls, user_query: str, max_docs: int = None, use_chunks: bool = True
    ) -> str:
        """
        ...
        try:
            ...
            logger.info(
                f"Hybrid search returned {len(documents) if documents else 0} documents (total: {total_count}) for query: {user_query}"
            )

            if not documents:
                logger.info(
                    f"Keine relevanten Dokumente für Query gefunden: {user_query}"
                )
                return ""
            ...
            logger.error(
                f"Fehler beim Erstellen des Knowledge-Kontexts: {str(e)}", exc_info=True
            )
            return ""
    ...
    def get_context_stats(cls, user_query: str) -> Dict:
        ...
        try:
            ...
            documents, total_count = SearchService.search_documents(
                query=user_query, page=1, per_page=10, active_only=True
            )
