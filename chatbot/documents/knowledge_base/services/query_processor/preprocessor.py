    ...
    def preprocess_for_fts5(cls, query: str, enable_medical_boost: bool = True) -> str:
        ...
        if enable_medical_boost:
            try:
                ...
                medical_synonyms = MedicalSynonymExpander.expand_query_with_synonyms(query)

                logger.info(f"Medical Boost: ICD={icd_codes}, Synonyms={medical_synonyms[:3]}")

            except ImportError:
                logger.warning("medical_extractors not available, skipping medical boost")
        ...
        stopwords = {'und', 'der', 'die', 'das', 'ist', 'in', 'zu', 'den', 'dem', 'von',
                     'für', 'auf', 'mit', 'als', 'bei', 'es', 'an', 'um', 'am', 'im',
                     'the', 'a', 'an', 'and', 'or', 'but', 'in', 'on', 'at', 'to', 'for'}

        words = [w for w in escaped_query.split() if len(w) >= 3 and w.lower() not in stopwords]
