...
class QueryAnalyzer:
    ...
    def extract_keywords(cls, query: str) -> List[str]:
        ...
        compound_pattern = re.findall(r'\b[a-zA-ZäöüÄÖÜß]+-[a-zA-ZäöüÄÖÜß]+\b', query.lower())
        for compound in compound_pattern:
            ...
            for part in parts:
                if len(part) >= 3 and part not in GERMAN_STOPWORDS and part not in keywords:
    ...
    def extract_entities(cls, query: str) -> List[str]:
        ...
        for i, word in enumerate(words):
            ...
            if (i > 0 and clean_word and clean_word[0].isupper() and 
                clean_word.lower() not in GERMAN_STOPWORDS):
    ...
    @classmethod
    def categorize_terms(cls, keywords: List[str], entities: List[str], 
                        intent: QueryIntent) -> Tuple[List[str], List[str]]:
        ...
        if intent in [QueryIntent.DEFINITION, QueryIntent.EXPLANATION, QueryIntent.SPECIFIC]:
