...
def save_file(
    ...
    try:
        ...
        if existing_doc:
            return (
                False,
                f"Dokument bereits vorhanden: {existing_doc.original_filename}",
                existing_doc,
            )
        ...
        if extension == "md":
            ...
            if extracted_metadata:
                ...
                description = (
                    extracted_metadata.get("description", description) or description
                )
        ...
        with get_db_connection() as conn:
            ...
            if (
                extension == "md"
                and "frontmatter_tags" in locals()
                and frontmatter_tags
            ):
            ...
            logger.info(
                f"Dokument erfolgreich gespeichert: {original_filename} -> {safe_filename}"
            )
