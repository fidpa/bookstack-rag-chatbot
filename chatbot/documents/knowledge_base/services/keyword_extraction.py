...
class KeywordExtractor:
    ...
    def extract_keywords(
        self, text: str, max_keywords: int = 20, include_ngrams: bool = True
        ...
        sorted_keywords = sorted(
            tf_idf_scores.items(), key=lambda x: x[1], reverse=True
        )
    ...
    def _extract_ngrams(self, tokens: List[str], n: int) -> List[str]:
        ...
        for i in range(len(tokens) - n + 1):
            ...
            if (
                len(non_stopwords) >= n // 2
            ):  # Mindestens die Hälfte sollten keine Stopwords sein
    ...
    def _calculate_tf_idf(self, terms: List[str], document: str) -> Dict[str, float]:
        ...
        for term, tf in tf_scores.items():
            ...
            caps_boost = (
                1.2
                if any(c.isupper() for c in document if term in document.lower())
                else 1.0
            )
    ...
    def extract_entities(self, text: str) -> Dict[str, List[str]]:
        ...
        number_pattern = re.compile(
            r"\b\d+(?:\.\d+)?\s*(?:€|Euro|EUR|USD|GB|MB|KB|%|km|m|cm|mm|kg|g|mg)\b"
        )
