...
class BookStackChunkingService:
    ...
    def __init__(self, chunk_size: int = None, overlap: int = None):
        ...
        if self.chunk_size < self.DEFAULTS["min_size"]:
            raise ValueError(
                f"chunk_size muss mindestens {self.DEFAULTS['min_size']} sein"
            )

        logger.info(
            f"BookStack Chunking Service initialisiert: {self.chunk_size} Wörter, {self.overlap} Overlap"
        )
    ...
    def chunk_bookstack_content(
        ...
        if not text or not text.strip() or len(text.strip()) < 10:
            logger.debug(
                f"Skipping empty/minimal content for BookStack {content_type} {bookstack_id} (length: {len(text.strip()) if text else 0})"
            )
            return []
        ...
        if not sentences:
            logger.warning(
                f"Keine Sätze in BookStack {content_type} {bookstack_id} gefunden"
            )
            return []
        ...
        logger.info(
            f"BookStack {content_type} {bookstack_id} in {len(chunks)} Chunks aufgeteilt"
        )
        return chunks
    ...
    def _optimize_chunks(self, chunks: List[BookStackChunk]) -> List[BookStackChunk]:
        ...
        while i < len(chunks):
            ...
            if (
                current_chunk.word_count < self.DEFAULTS["min_size"]
                and i < len(chunks) - 1
            ):
                ...
                if (
                    combined_word_count <= self.chunk_size * 1.3
                ):  # 30% Toleranz für BookStack
                    # Chunks mergen
    ...
    def validate_chunks(self, chunks: List[BookStackChunk]) -> Tuple[bool, List[str]]:
        ...
        for i, chunk in enumerate(sorted_chunks):
            ...
            if chunk.chunk_index != i:
                errors.append(
                    f"Chunk hat falschen Index: erwartet {i}, gefunden {chunk.chunk_index}"
                )

            # Prüfe BookStack-spezifische Felder
            if chunk.content_type not in ["page", "chapter", "book"]:
                errors.append(
                    f"Chunk {i} hat ungültigen content_type: {chunk.content_type}"
                )

            if chunk.bookstack_id <= 0:
                errors.append(
                    f"Chunk {i} hat ungültige bookstack_id: {chunk.bookstack_id}"
                )
