...
class WidgetSessionManager:
    ...
    def get_or_create_session(session_id: Optional[str] = None) -> str:
        ...
        expired = [
            sid
            for sid, data in widget_sessions.items()
    ...
    def add_message(session_id: str, role: str, content: str):
        ...
        if session_id in widget_sessions:
            ...
            if len(widget_sessions[session_id]["messages"]) > 20:
                widget_sessions[session_id]["messages"] = widget_sessions[session_id][
                    "messages"
                ][-20:]
...
def process_widget_message(
    ...
    try:
        ...
        if bookstack_context:
            ...
            logger.info(
                f"Updated context for session {session_id}: {bookstack_context.get('title', 'Unknown')}"
            )
        ...
        ai_response, provider_info = generate_widget_response(
            message, session_id, bookstack_context
        )
    ...
    try:
        ...
        if not provider and ENABLE_OLLAMA:
            try:
                ...
                if provider.is_available():
                    logger.info(
                        "Using Ollama provider for widget (ENABLE_OLLAMA_FALLBACK=true)"
                    )
                else:
                    provider = None
            except Exception as e:
        ...
        if not provider:
            if ENABLE_OLLAMA:
                logger.error(
                    "No LLM provider available (Azure and Ollama both unreachable)"
                )
                return (
                ...
                return (
                    "Azure OpenAI is not available. Please check the configuration.",
                    {"provider": "error", "model": None},
                )
        ...
        if bookstack_context:
            ...
            context_parts.append(
                f"User is viewing the '{page_name}' page in the '{book_name}' book"
            )
            ...
            combined_context = ChatContextBuilder.build_combined_context(
                enhanced_query, bookstack_context
            )
        else:
        ...
        for msg in messages[-10:]:
            if msg["role"] in ["user", "assistant"]:
                ...
                if (
                    msg["role"] == "user"
                    and msg["content"] == user_message
                    and bookstack_context
                ):
                    ...
                    llm_messages.append(
                        {"role": msg["role"], "content": enhanced_content}
                    )
                else:
                    llm_messages.append(
        ...
        logger.info(
            f"Widget LLM Request - System Prompt: {widget_system_prompt[:100]}..."
        )
        ...
        provider_info = {
            ...
            "model": (
                getattr(provider, "model", None) if hasattr(provider, "model") else None
            ),
        }

        logger.info(
            f"Generated widget response using {provider_info['provider']} for session {session_id}"
        )
